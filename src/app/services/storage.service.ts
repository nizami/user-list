...
import { UserDto } from './users.api.service';
...
export class StorageService {
  ...
  public getUsers(): UserDto[] {
    ...
    if (storageItem) {
      return JSON.parse(storageItem) as UserDto[];
    }

    return DB;
  }

  public saveUsers(users: UserDto[]): void {
    const storageItem = JSON.stringify(users);
    localStorage.setItem(this.storageKey, storageItem);
  }
